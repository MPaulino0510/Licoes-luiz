namespace Tabuada_fácil
{
    internal class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine($"1 * 1 = {1 * 1}");
            Console.WriteLine($"1 * 2 = {1 * 2}");
            Console.WriteLine($"1 * 3 = {1 * 3}");
            Console.WriteLine($"1 * 4 = {1 * 4}");
            Console.WriteLine($"1 * 5 = {1 * 5}");
            Console.WriteLine($"1 * 6 = {1 * 6}");
            Console.WriteLine($"1 * 7 = {1 * 7}");
            Console.WriteLine($"1 * 8 = {1 * 8}");
            Console.WriteLine($"1 * 9 = {1 * 9}");
            Console.WriteLine($"1 * 10 = {1 * 10}");

            Console.WriteLine($"2 * 1 = {2 * 1}");
            Console.WriteLine($"2 * 2 = {2 * 2}");
            Console.WriteLine($"2 * 3 = {2 * 3}");
            Console.WriteLine($"2 * 4 = {2 * 4}");
            Console.WriteLine($"2 * 5 = {2 * 5}");
            Console.WriteLine($"2 * 6 = {2 * 6}");
            Console.WriteLine($"2 * 7 = {2 * 7}");
            Console.WriteLine($"2 * 8 = {2 * 8}");
            Console.WriteLine($"2 * 9 = {2 * 9}");
            Console.WriteLine($"2 * 10 = {2 * 10}");

            Console.WriteLine($"3 * 1 = {3 * 1}");
            Console.WriteLine($"3 * 2 = {3 * 2}");
            Console.WriteLine($"3 * 3 = {3 * 3}");
            Console.WriteLine($"3 * 4 = {3 * 4}");
            Console.WriteLine($"3 * 5 = {3 * 5}");
            Console.WriteLine($"3 * 6 = {3 * 6}");
            Console.WriteLine($"3 * 7 = {3 * 7}");
            Console.WriteLine($"3 * 8 = {3 * 8}");
            Console.WriteLine($"3 * 9 = {3 * 9}");
            Console.WriteLine($"3 * 10 = {3 * 10}");

            Console.WriteLine($"4 * 1 = {4 * 1}");
            Console.WriteLine($"4 * 2 = {4 * 2}");
            Console.WriteLine($"4 * 3 = {4 * 3}");
            Console.WriteLine($"4 * 4 = {4 * 4}");
            Console.WriteLine($"4 * 5 = {4 * 5}");
            Console.WriteLine($"4 * 6 = {4 * 6}");
            Console.WriteLine($"4 * 7 = {4 * 7}");
            Console.WriteLine($"4 * 8 = {4 * 8}");
            Console.WriteLine($"4 * 9 = {4 * 9}");
            Console.WriteLine($"4 * 10 = {4 * 10}");

            Console.WriteLine($"5 * 1 = {5 * 1}");
            Console.WriteLine($"5 * 2 = {5 * 2}");
            Console.WriteLine($"5 * 3 = {5 * 3}");
            Console.WriteLine($"5 * 4 = {5 * 4}");
            Console.WriteLine($"5 * 5 = {5 * 5}");
            Console.WriteLine($"5 * 6 = {5 * 6}");
            Console.WriteLine($"5 * 7 = {5 * 7}");
            Console.WriteLine($"5 * 8 = {5 * 8}");
            Console.WriteLine($"5 * 9 = {5 * 9}");
            Console.WriteLine($"5 * 10 = {5 * 10}");

            Console.WriteLine($"6 * 1 = {6 * 1}");
            Console.WriteLine($"6 * 2 = {6 * 2}");
            Console.WriteLine($"6 * 3 = {6 * 3}");
            Console.WriteLine($"6 * 4 = {6 * 4}");
            Console.WriteLine($"6 * 5 = {6 * 5}");
            Console.WriteLine($"6 * 6 = {6 * 6}");
            Console.WriteLine($"6 * 7 = {6 * 7}");
            Console.WriteLine($"6 * 8 = {6 * 8}");
            Console.WriteLine($"6 * 9 = {6 * 9}");
            Console.WriteLine($"6 * 10 = {6 * 10}");

            Console.WriteLine($"7 * 1 = {7 * 1}");
            Console.WriteLine($"7 * 2 = {7 * 2}");
            Console.WriteLine($"7 * 3 = {7 * 3}");
            Console.WriteLine($"7 * 4 = {7 * 4}");
            Console.WriteLine($"7 * 5 = {7 * 5}");
            Console.WriteLine($"7 * 6 = {7 * 6}");
            Console.WriteLine($"7 * 7 = {7 * 7}");
            Console.WriteLine($"7 * 8 = {7 * 8}");
            Console.WriteLine($"7 * 9 = {7 * 9}");
            Console.WriteLine($"7 * 10 = {7 * 10}");

            Console.WriteLine($"8 * 1 = {8 * 1}");
            Console.WriteLine($"8 * 2 = {8 * 2}");
            Console.WriteLine($"8 * 3 = {8 * 3}");
            Console.WriteLine($"8 * 4 = {8 * 4}");
            Console.WriteLine($"8 * 5 = {8 * 5}");
            Console.WriteLine($"8 * 6 = {8 * 6}");
            Console.WriteLine($"8 * 7 = {8 * 7}");
            Console.WriteLine($"8 * 8 = {8 * 8}");
            Console.WriteLine($"8 * 9 = {8 * 9}");
            Console.WriteLine($"8 * 10 = {8 * 10}");

            Console.WriteLine($"9 * 1 = {9 * 1}");
            Console.WriteLine($"9 * 2 = {9 * 2}");
            Console.WriteLine($"9 * 3 = {9 * 3}");
            Console.WriteLine($"9 * 4 = {9 * 4}");
            Console.WriteLine($"9 * 5 = {9 * 5}");
            Console.WriteLine($"9 * 6 = {9 * 6}");
            Console.WriteLine($"9 * 7 = {9 * 7}");
            Console.WriteLine($"9 * 8 = {9 * 8}");
            Console.WriteLine($"9 * 9 = {9 * 9}");
            Console.WriteLine($"9 * 10 = {9 * 10}");

            Console.WriteLine($"10 * 1 = {10 * 1}");
            Console.WriteLine($"10 * 2 = {10 * 2}");
            Console.WriteLine($"10 * 3 = {10 * 3}");
            Console.WriteLine($"10 * 4 = {10 * 4}");
            Console.WriteLine($"10 * 5 = {10 * 5}");
            Console.WriteLine($"10 * 6 = {10 * 6}");
            Console.WriteLine($"10 * 7 = {10 * 7}");
            Console.WriteLine($"10 * 8 = {10 * 8}");
            Console.WriteLine($"10 * 9 = {10 * 9}");
            Console.WriteLine($"10 * 10 = {10 * 10}");
        }
    }
}
    
